...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans selection:bg-indigo-500/30 overflow-y-auto overscroll-contain touch-pan-y pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      ...
      {uiState.showCreateProfile || (userState.profiles.length === 0 && !userState.activeProfile) ? (
        <CreateProfileScreen onCreated={handleProfileCreated} />
      ) : !userState.activeProfile ? (
        <WelcomeScreen 
          profiles={userState.profiles} 
          onSelect={handleProfileSelect} 
          onCreateNew={() => setUiState(p => ({ ...p, showCreateProfile: true }))} 
          savedRoutes={allRecentTrips || []} 
